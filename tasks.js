...
export function loadDutiesForVerification() {
  ...
  dutyInputs.forEach(dutyInput => {
    ...
    container.insertAdjacentHTML('beforeend', createDutyAccordion(dutyId, dutyText, tasks));
  });
...
export function updateWorkshopCount(taskKey, dimension, value, count) {
  if (!appState.workshopCounts[taskKey]) {
    ...
    if (!dutyTitle) {
      ...
      dutyId        = parts[0];
      const dutyEl  = document.querySelector(`[data-duty-id="${dutyId}"]`);
      dutyTitle     = dutyEl ? dutyEl.value.trim() : '';
      const taskEl  = document.querySelector(`[data-task-id="${taskKey}"]`);
  ...
  Object.keys(resultsSource || {}).forEach(taskKey => {
    ...
    if (result && result.valid) {
      ...
      if (!dutyText || !taskText) {
        ...
        if (!dutyText) { const el = document.querySelector(`[data-duty-id="${dutyId}"]`); dutyText = el ? el.value.trim() : 'Unassigned Duty'; }
        if (!taskText) { const el = document.querySelector(`[data-task-id="${taskKey}"]`); taskText = el ? el.value.trim() : 'Unassigned Task'; }
      }
      validResults.push({ duty: dutyText, task: taskText, meanImportance: result.meanImportance, meanFrequency: result.meanFrequency, meanDifficulty: result.meanDifficulty, priorityIndex: result.priorityIndex });
    }
  });
...
function updateDutyLevelSummaryFromSource(resultsSource) {
  ...
  Object.keys(resultsSource).forEach(taskKey => {
    ...
    if (result && result.valid) {
      ...
      if (!dutyTitle) { const el = document.querySelector(`[data-duty-id="${dutyId}"]`); dutyTitle = el ? el.value.trim() : 'Unassigned Duty'; }
      if (!dutyMap[dutyId]) dutyMap[dutyId] = { dutyTitle, totalTasks:0, validTasks:0, importanceSum:0, frequencySum:0, difficultySum:0, prioritySum:0, tasks:[] };
  ...
  Object.keys(dutyMap).forEach(dutyId => {
    ...
    if (vc > 0) {
      ...
      dutyResults.push({ dutyTitle: duty.dutyTitle, totalTasks: duty.totalTasks, validTasks: vc, avgImportance: avgI, avgFrequency: avgF, avgDifficulty: avgD, avgPriority: avgP, highPriorityCount: highPC, trainingLoad });
    }
  });
...
export function exportDashboard() {
  ...
  Object.keys(appState.workshopResults).forEach(taskKey => {
    ...
    if (r && r.valid) {
      ...
      validResults.push({ duty: dutyText, task: taskText, meanImportance: r.meanImportance, meanFrequency: r.meanFrequency, meanDifficulty: r.meanDifficulty, priorityIndex: r.priorityIndex });
    }
  });
  validResults.sort((a,b) => b.priorityIndex - a.priorityIndex);
  let csv = 'Rank,Duty,Task,Mean Importance,Mean Frequency,Mean Difficulty,Priority Index\n';
  validResults.forEach((r,i) => {
    csv += `${i+1},"${r.duty.replace(/"/g,'""')}","${r.task.replace(/"/g,'""')}",${r.meanImportance.toFixed(2)},${r.meanFrequency.toFixed(2)},${r.meanDifficulty.toFixed(2)},${r.priorityIndex.toFixed(2)}\n`;
  });
